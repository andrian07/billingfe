...
class CafeReportPage extends StatefulWidget {
  ...
  @override
  State<CafeReportPage> createState() => _CafeReportPageState();
}
...
class _CafeReportPageState extends State<CafeReportPage> {
  ...
  Future<void> _export(String type) async {
    ...
    try {
      ...
      final savePath = await FilePicker.platform.saveFile(
        dialogTitle: "Simpan Laporan Cafe",
  ...
  Widget build(BuildContext context) {
    return AppLayout(
      title: "Laporan Cafe",
      subtitle: "Rekap transaksi cafe/POS",
      showSearch: false,
  ...
  Widget _buildBody() {
    ...
    if (result == null || result.rows.isEmpty) {
      return _buildMessageState(
        ...
        message: "Tidak ada transaksi cafe pada rentang tanggal ini",
      );
    }
    ...
    return Column(
      children: [
        Padding(
          ...
          child: _CafeReportRowWidget.header(),
        ),
        Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (context, index) =>
                _CafeReportRowWidget.data(no: index + 1, row: result.rows[index]),
          ),
        ),
        const Divider(height: 1, color: AppColors.divider),
        Container(
...
class _CafeReportRowWidget extends StatelessWidget {
  final bool header;
  final int? no;
  final CafeReportRow? row;

  const _CafeReportRowWidget.header() : header = true, no = null, row = null;

  const _CafeReportRowWidget.data({required this.no, required this.row})
    : header = false;
